...
/**
 * ResourcePatternResolver的实现类，可以一次加载多个资源
 * 并把结果以数组形式返回。
 *
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	// 直接委托给 ResourceLoader 来实现，如果使用无参构造方法，则是使用 DefaultResourceLoader 来加载资源
	@Override
	...
	protected Resource[] findAllClassPathResources(String location) throws IOException {
		...
		// 真正执行加载所有 classpath 资源  doFindAllClassPathResources 方法
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		/**
		 * 通过 ClassLoader 加载路径下的所有资源
		 * 而在 ClassLoader 的 getResources 方法中有一段：
		 *
		 * 		if (parent != null) {
